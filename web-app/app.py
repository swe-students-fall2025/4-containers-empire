...
def create_app():
    ...
    def home():
        ...
        recent = list(photos_collection.find({"user_id": user_id}).sort("uploaded_at", -1).limit(5))
    ...
    def register():
        if request.method == "POST":
            ...
            users_collection.insert_one({"username": username, "hashed": hashed, "email": email})
    ...
    def upload():
        if request.method == "POST":
            ...
            if file:
                ...
                data = file.read()
                photos_collection.insert_one({"user_id": str(current_user.get_id()), "filename": filename, "data": data, "uploaded_at": datetime.utcnow(), "species": None, "status": "pending"})
    ...
    def my_animals():
        user_id = str(current_user.get_id())
        observations = list(photos_collection.find({"user_id": user_id}).sort("uploaded_at", -1))
    ...
    def get_image(img_id):
        try:
            ...
            if not img_doc:
                abort(404)
            return send_file(io.BytesIO(img_doc["data"]), attachment_filename = img_doc["filename"], mimetype = "image/jpeg")
        except Exception:
            abort(404)
